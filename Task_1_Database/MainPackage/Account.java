...
public abstract class Account
{
	...
	private 		 DB_Handler d = new DB_Handler();
	...
	public void makeDeposit(int amount)
	{
		...
		if ( amount > 0)
		{
			...
			d.addstatement(accountno,"You Deposited " + amount + " to Account " + accountno + " Date: " + LocalDateTime.now());
			d.updatebalanceChecking(accountno, balance);
			d.updatebalanceSaving(accountno, balance);
		}
		else
		{
	...
	public void makeWithdrawal(int amount)
	{
		...
		if ( amount <= balance)
		{
			...
			d.addstatement(accountno, "You Withdraw  " + amount + " from Account " + accountno + " Date: " + LocalDateTime.now());
			d.updatebalanceChecking(accountno, balance);
			d.updatebalanceSaving(accountno, balance);
		}
		else
			...
			if (accountno > 0)
			{
				...
				for(Customer a : ac)
				{
					if (a.getAccount(1)!=null &&  a.getAccount(1).getAccountno() == accountno)
					{
						...
						d.addstatement(accountno, "You Transferred " + amount + " to Account " + accountno + " Date: " + LocalDateTime.now());
						d.addstatement(a.getAccount(1).getAccountno(),"Account " + this.accountno + " Deposited " + amount + " to Your Account on Date: " + LocalDateTime.now() );
						
						// Update Balance
						d.updatebalanceChecking(accountno, balance);
						d.updatebalanceSaving(accountno, balance);
						
						d.updatebalanceChecking(a.getAccount(1).getAccountno(), balance);
						d.updatebalanceSaving(a.getAccount(1).getAccountno(), balance);
						break;
					}
					else if (a.getAccount(2)!=null && a.getAccount(2).getAccountno() == accountno)
					{
						...
						statement.add( "You Transferred " + amount + " to Account " + accountno + " Date: " + LocalDateTime.now());
						d.addstatement(accountno, "You Transferred " + amount + " to Account " + accountno + " Date: " + LocalDateTime.now());
						d.addstatement(a.getAccount(2).getAccountno(),"Account " + this.accountno + " Deposited " + amount + " to Your Account on Date: " + LocalDateTime.now() );

						// UPdate Balance
						
						d.updatebalanceChecking(accountno, balance);
						d.updatebalanceSaving(accountno, balance);
						d.updatebalanceChecking(a.getAccount(2).getAccountno(), balance);
						d.updatebalanceSaving(a.getAccount(2).getAccountno(), balance);
						break;
					}
				}
				if (status == 1)
	...
	public void calculateZakat()
	{
		...
		if (choice == 1 )
		{
			...
			d.adddeduction(accountno, "Zakat " + amount + " paid successfully on " + LocalDateTime.now() );
			checkBalance();
			d.updatebalanceChecking(accountno, balance);
			d.updatebalanceSaving(accountno, balance);
		}
